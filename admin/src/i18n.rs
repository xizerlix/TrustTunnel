...
pub const EN: I18n = I18n {
    ...
    totp_help: "TOTP: a 6-digit code from Google Authenticator, Aegis, or Authy after the password. The secret is stored only in admin.toml on this server (mode 0600), never in git. If you lose the phone, set totp_enabled = false in that file and restart the admin.",
    ...
    totp_uri_label: "otpauth URI (paste or scan)",
...
pub const RU: I18n = I18n {
    ...
    totp_help: "TOTP: после пароля нужен 6-значный код из Google Authenticator, Aegis или Authy. Секрет хранится только в admin.toml на этом сервере (права 0600), в git его нет. Потеряли телефон — в том файле поставьте totp_enabled = false и перезапустите админку.",
    ...
    totp_uri_label: "otpauth URI (вставить или сканировать)",
